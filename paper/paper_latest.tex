...
% Springer author-year citations are "(Name 2026)", with no comma before the year;
% paper_conference.sty sets aysep={,}, so this has to come after it is loaded.
\setcitestyle{aysep={}}

% Springer caption labels: "Fig. 1" / "Table 1", bold, separated by a space rather
% than a colon. Scoped to figure/table so subcaption's (a)/(b) labels are untouched.
...
\author{%
  Victor Gao\thanks{Equal contribution.}\ \ \thanks{Persis Capital Inc.,
  Toronto, Ontario, Canada.} \quad
...
% Not an ICLR submission. paper_conference.sty sets the venue line as \lhead inside
...
\noindent\textbf{Keywords}\quad Large language models \textperiodcentered\ Multi-agent
systems \textperiodcentered\ Inference-time scaling \textperiodcentered\ Code generation
\textperiodcentered\ Self-orchestration \textperiodcentered\ LiveCodeBench
\end{minipage}
\end{center}
\vskip 0.2in
...
shared store rather than a pipeline - ARIADNE \citep{wei2026} pairs a blackboard with Monte
...
% paper_conference.sty, \pagebreak stretches the short page's glue instead of leaving the gap.
\newpage
...
\paragraph{Statistics.} Reported $\pm$SD is the sample standard deviation (SD; divisor $n-1 = 4$) of the five
...
\paragraph{Generative AI tools.} Language models are the object of study here: every
reported number is a measurement of model output produced by the scaffold and baselines
above. Generative language-model tools were additionally used to assist in preparing this
work, and no reported result was generated, imputed or altered by them; the Declarations
...
\paragraph{Reasoning on:} Opus-5 scores at $88$ single $\to 95$ manager, the highest of all tested scenarios. However, smaller models tend to gain more from the manager (Fig.~\ref{fig:scale-bars}a).
...
eliminates truncation and reproduces the ordering (panel c; Fig.~\ref{fig:scale-off}).
...
This study involves no human participants, no animal subjects, and no
personal data, so no ethics approval and no informed consent were required.
...
The evaluation set is the public hard split of LiveCodeBench release\_v6 \citep{jain2024},
obtainable from that benchmark's own distribution; this study released no new dataset.

\iffinalversion
The data generated and analysed during this study, the orchestration scaffold (both
versions), the corrected LiveCodeBench evaluator, and the
...
bounded by its 262{,}144-token context window \citep{qwen2026c} shared by prompt and output,
...
scored on the same evaluator as every other cell (Appendix~\ref{app:evaluator}): 
