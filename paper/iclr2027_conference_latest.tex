...
points, to 70.0 (Table~\ref{tab:qwen-caps}). Re-scoring each single arm over only the problem-passes
...
whose style trips it, and the $\Delta$ harness column of Table~\ref{tab:regrade} ranges from
$+0.0$ for Fable 5, which never writes it, to $+12.6$ for GPT-5.6-Terra's manager arm.
...
evaluator marked wrong, so neither can turn a pass into a failure and every move in
Table~\ref{tab:regrade} raises a score rather than lowering one; and among those they re-run only
