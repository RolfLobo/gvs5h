...
The GVS5H scaffold improves the coding accuracy of each pinned-backend model ran in both arms. These results provide a baseline against which more complex systems involving heterogeneous models or code verification can be compared.
...
attributes the gain to decomposition and to context that persists across calls.   The result is conditional - some configurations result no improvement or regression. However, in many cases, the orchestration is a cost-effective substitute for model scale.
...
number is re-scored all the same, and one cell moves: Kimi-K3's 128k reasoning-on single
arm, from 82 to 83. Because the idiom is a matter of coding style, a harness bug of this shape is
not a wash across a leaderboard --- it penalizes the models whose style trips it.
