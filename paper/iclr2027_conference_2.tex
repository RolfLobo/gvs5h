...
models on the 100 latest hard LiveCodeBench problems, the method yields gains as much as
23.2 percentage points and boosts two cheaper models to frontier-level
performance. Orchestrated GPT-5.6-Terra reaches 88.0\% pass@1 against
Fable 5's 90.4\% at 19\%
of the cost, and locally served, open-weight Qwen3.8-27B rises from 69.2\% to 92.4\%. Gains
...
by the extractor. Of the single arm's 35 empty cells (Table~\ref{tab:emitted}), the manager
solves 28, fails 7 and leaves none empty, worth 5.6 points, about a quarter of its gain --- a
cut-off worker costs a round rather than the answer. Served locally at a 262{,}144-token context, Qwen3.8-27B's
...
\paragraph{Reasoning on.} Opus-5 tops the study at $88 \to 95$, its $+7$ seven problems gained
and none lost (Table~\ref{tab:openrouter}). The largest gains come from the smaller models (Figure~\ref{fig:scale-bars}a):
...
\begin{itemize}
  ...
  time-varying confound: providers intermittently stalled, dropped the response
  ...
  five-pass runs. It is also why Opus-5's $88 \to 95$ (Table~\ref{tab:openrouter}), the highest
  score in the paper, remains a point estimate rather than a measured effect.
  ...
  variance rather than a constant offset. Comparisons against Fable 5 are therefore mildly
  ...
  generations either side; we report the as-generated 250k reading
...
\textsuperscript{b}~A hard ceiling rather than a chosen setting: the GPT-5.6 family and
